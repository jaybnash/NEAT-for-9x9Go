...
def eval_genome_vs_genome_batch(genome_pairs_batch, config):
    ...
    for genome1, genome2 in genome_pairs_batch:
        # Create CPPNs
        cppn1 = neat.nn.FeedForwardNetwork.create(genome1, config)
        cppn2 = neat.nn.FeedForwardNetwork.create(genome2, config)
        # Create ESNetworks
        es_net1 = es_hyperneat.ESNetwork(sub, cppn1, params)
        es_net2 = es_hyperneat.ESNetwork(sub, cppn2, params)
        # Create phenotype networks
        phenotype_net1 = es_net1.create_phenotype_network()
        phenotype_net2 = es_net2.create_phenotype_network()
        phenotype_net1_list.append(phenotype_net1)
        phenotype_net2_list.append(phenotype_net2)
